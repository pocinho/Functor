...
type EditorControl() =
    ...
    member private this.PositionAtPoint(point: Point) =
        LayoutEngine.positionAtPoint
            ...
            session.Model.HorizontalOffset
            session.Model.VerticalOffset
    ...
    member private this.CopySelection() =
        match EditingLogic.selectedText session.Model.Editing, TopLevel.GetTopLevel(this) with
    ...
    member this.VerticalOffset = session.Model.VerticalOffset
    ...
    member this.ScrollTo(offset: int) =
        ...
        session.Dispatch(CoreEvent.ScrollTo clamped)
        this.InvalidateVisual()
        this.NotifyScrollStateChanged()
    ...
    member private this.UpdateViewport() =
        ...
        session.Dispatch(CoreEvent.ResizeViewport(int b.Width, int b.Height))
