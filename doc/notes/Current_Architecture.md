...
`ShellHostView` contains the editor, welcome surface, document tabs, a global left tool rail and tool host, a separate active-document Agent host, scrollbars, and status bar. It subscribes to editor state, status, and scroll notifications only while attached to the visual tree. On each notification it builds `ShellViewInput` through `ShellProjection` and applies it through `ShellView`.

`PerDocumentSessionState.Auxiliary` owns the live Agent visibility choice for each document tab. `EditorSession` toggles it through the command-palette `ToggleAgentPanel` command, and `ShellProjection` derives the active tab's Agent choice for the right-side auxiliary host. The Workspace explorer is a global left tool and no longer opens a duplicate document-specific host. Notebook execution and agent/MCP integration remain future adapters.
