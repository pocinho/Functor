...
`ShellHostView` contains the editor, welcome surface, document tabs, scrollbars, status bar, panel rail, and `SidePanelView`. It subscribes to editor state, status, and scroll notifications only while attached to the visual tree. On each notification it builds `ShellViewInput` through `ShellProjection` and applies it through `ShellView`.

`ShellModel` is a pure model for side-panel state: open/closed status, bounded width, selected panel, and animation flag. `ShellUpdate.update` handles shell messages and can emit an `AppCommand` effect for file commands. `ShellViewNode.describe` describes the editor, welcome, tab, status, scroll, and side-panel presentation states. `ShellView` currently materializes the side panel with placeholder `TextBlock` content for the `Notebook`, `Agent`, and named plugin panel variants; it does not integrate notebook, agent, or plugin functionality.
